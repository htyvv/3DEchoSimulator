...
from .utils import Tags, TissueTagNames, BloodTagNames, OtherTagNames
...
def process_output_image(img, include_blood=True):
    ...
    max_val = img.max()
    if max_val > 255:
        img = img * (255.0 / max_val)  # normalize
        final_vals = {k: v * 255.0 / max_val for k, v in final_vals.items()}
